"""
Build rolling-window features + total-games target for tennis total games model.
Replays tennis_matches chronologically, computing PRE-MATCH-ONLY rolling stats
per player (5/10/20-match windows, streaks, 90-day stats, surface form),
then parses the score string to compute the actual total games played.
Writes everything into tennis_match_features using batched inserts for speed.
"""
import os
import re
import sys
...
def main():
    ...
    total_rows = len(rows)
    print(f"Loaded {total_rows} matches")
    ...
    for row in rows:
        ...
        if len(batch) >= BATCH_SIZE:
            ...
            conn.commit()
            batch = []
            print(f"Processed {processed}/{total_rows} matches")

    if batch:
        psycopg2.extras.execute_values(cur, INSERT_SQL, batch)
        conn.commit()

    print(f"Done. Processed {processed} matches.")
    cur.close()
    read_cur.close()
    conn.close()
