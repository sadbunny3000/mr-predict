"""
Compares three quantile-regression approaches on the same real features
and same train/test split:
  1. GradientBoostingRegressor (already trained as tennis_total_games_model_v3.pkl)
  2. XGBoost (reg:quantileerror)
  3. LightGBM (objective='quantile')
Whichever wins per segment becomes the recommended model for that segment.
"""
import os
import pickle
import random
...
V3_MODEL_PATH = 'backend/ml/saved_models/tennis_total_games_model_v3.pkl'
OUTPUT_PATH   = 'backend/ml/saved_models/tennis_total_games_model_v4_ensemble.pkl'
...
def main():
    ...
    final_models = {}
    print(f"{'Segment':<12} {'GBR(v3)':>10} {'XGBoost':>10} {'LightGBM':>10} {'Winner':>10}")
    ...
    for seg in SEGMENTS:
        ...
        if len(seg_train) < 50 or not seg_test:
            print(f"{seg:<12} skipped (insufficient data)")
            continue
        ...
        # --- GBR (v3, already trained) ---
        gbr_median = v3_models[seg]['quantile_models'][0.50]
        gbr_preds = gbr_median.predict(X_test)
        gbr_mae, gbr_rmse = mae_rmse(gbr_preds, y_test)
        ...
        for q in QUANTILES:
            ...
            xgb_models[q] = model
        xgb_preds = xgb_models[0.50].predict(X_test)
        xgb_mae, xgb_rmse = mae_rmse(xgb_preds, y_test)
        ...
        for q in QUANTILES:
            ...
            model.fit(X_train, y_train)
            lgb_models[q] = model
        lgb_preds = lgb_models[0.50].predict(X_test)
        lgb_mae, lgb_rmse = mae_rmse(lgb_preds, y_test)

        scores = {'GBR': gbr_mae, 'XGBoost': xgb_mae, 'LightGBM': lgb_mae}
        winner = min(scores, key=scores.get)
        print(f"{seg:<12} {gbr_mae:>10.3f} {xgb_mae:>10.3f} {lgb_mae:>10.3f} {winner:>10}")
        ...
        if winner == 'XGBoost':
            ...
            final_models[seg] = v3_models[seg]
            final_models[seg]['model_type'] = 'gbr'

    with open(OUTPUT_PATH, 'wb') as f:
        pickle.dump(final_models, f)
    print(f"\nSaved best-per-segment ensemble to {OUTPUT_PATH}")
